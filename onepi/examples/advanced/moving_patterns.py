...
def compute_pulses_from_angle_and_curvature(angle_rad, radius_of_curvature_mm=0):
    abs_angle_rad = abs(angle_rad)
    ut = ControlUtils()
    arc_length_mm = ut.compute_arc_length(abs_angle_rad, radius_of_curvature_mm)
    revolutions = ut.compute_revolutions_from_distance(arc_length_mm)
    num_pulses = ut.compute_pulses_from_rev(revolutions)
    return num_pulses
...
def rotate_angle_deg_at_speed(
    ...
    total_pulses = compute_pulses_from_angle_and_curvature(
        math.radians(angle_deg), radius_of_curvature_mm
    )
    print_value("total_pulses: ", total_pulses)
    slow_down_pulses_thresh = compute_pulses_from_angle_and_curvature(
...
def move_straight_at_speed(distance, speed=50, slow_down_distance=0):
    ...
    total_pulses = compute_pulses_from_distance(abs_distance)
    slow_down_pulses = compute_pulses_from_distance(abs_slow_down_distance)
...
def read_encoder(pose_speeds: PoseSpeeds, radius_of_curvature_mm, dt):
    if radius_of_curvature_mm == STRAIGHT_MOTION:
        ...
        return compute_pulses_from_distance(distance)
    else:
        angle_rad = abs(pose_speeds.angular_rad) * dt
        return compute_pulses_from_angle_and_curvature(
